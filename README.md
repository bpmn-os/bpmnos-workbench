...
The controls a run is driven by stand in the panel's footer, under every column: run and pause with the speed beside them, and, to their right, beginning the run again, loading an execution log and saving the one a run produced. They govern the run rather than any one list, so they are reachable whichever column is open and whether any is; while the workbench is modelling there is no run and the footer is not there. Loading a log is offered during playback and greyed while a simulation produces its own.
